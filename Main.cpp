    ...
    while (window.isOpen()) {
        ...
        if (gameState == Gameplay) {
           ...
           if (cactusPassed.size() != cactusBounds.size()) {
            ...
            float playerRight = playerLeft + player.getGlobalBounds().width;

            for (size_t i = 0; i < cactusBounds.size(); ++i) {
                float cactusRight = cactusBounds[i].left + cactusBounds[i].width;
                
                if (!cactusPassed[i] && playerLeft > cactusRight) {
                    cactusCount++;
                    cactusPassed[i] = true; 
                }
            }
            ...
            for (size_t i = 0; i < birdBounds.size(); ++i) {
                float birdRight = birdBounds[i].left + birdBounds[i].width;
                
                if (!birdPassed[i] && playerLeft > birdRight) {
                    birdCount++;
                    birdPassed[i] = true;
                }
            }
