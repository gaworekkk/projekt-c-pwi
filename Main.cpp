...
int main() {
    ...
    while (window.isOpen()) {
        ...
        if (gameState == Gameplay) {
            ...
            float playerLeft = player.getGlobalBounds().left;
            float playerBottom = player.getGlobalBounds().top + player.getGlobalBounds().height;

            for (size_t i = 0; i < cactusBounds.size(); ++i) {
                if (!cactusPassed[i] && cactusBounds[i].left + cactusBounds[i].width < playerLeft && player.getVelocity().x >= 0 && playerBottom >= cactusBounds[i].top) {
                    cactusCount++;
                    cactusPassed[i] = true;
                }
            }
            for (size_t i = 0; i < birdBounds.size(); ++i) {
                if (!birdPassed[i] && birdBounds[i].left + birdBounds[i].width < playerLeft && player.getVelocity().x >= 0) {
                    birdCount++;
                    birdPassed[i] = true;
                }
            }
